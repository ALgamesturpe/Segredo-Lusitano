<?php
// SEGREDO LUSITANO — Redefinir Palavra-passe
...
$u = $st->fetch();
if (!$u) {
    unset($_SESSION['recuperar_id'], $_SESSION['recuperar_codigo_ok']);
    header('Location: ' . SITE_URL . '/pages/recuperar_password.php');
    exit;
}
...
$fase = empty($_SESSION['recuperar_codigo_ok']) ? 1 : 2;
$erro = '';

// ── Reenviar código (só na fase 1) ───────────────────────────
if (isset($_POST['reenviar'])) {
    ...
    flash($enviado ? 'success' : 'error', $enviado
        ? 'Novo código enviado para ' . $email_mask . '!'
        : 'Erro ao enviar email. Tenta novamente.');
    header('Location: ' . SITE_URL . '/pages/redefinir_password.php');
    exit;
}

// ── Fase 1: validar código ────────────────────────────────────
if ($fase === 1 && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['codigo'])) {
    verificar_csrf();
    ...
    if (strlen($codigo) !== 6) {
        ...
        $_SESSION['recuperar_codigo_ok'] = true;
        header('Location: ' . SITE_URL . '/pages/redefinir_password.php');
        exit;
    }
}

// ── Fase 2: guardar nova password ────────────────────────────
if ($fase === 2 && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['password'])) {
    verificar_csrf();
  ...
  <div class="form-container" style="max-width:460px;width:100%;">
    ...
    <div style="display:flex;justify-content:center;margin-bottom:2rem;">
      <img src="<?= SITE_URL ?>/assets/images/logo_icon.png" alt="Segredo Lusitano"
           style="height:80px;width:80px;object-fit:contain;filter:drop-shadow(0 0 10px rgba(201,168,76,.5));">
    </div>

    <?php if ($fase === 1): ?>

      <h1 class="form-title" style="text-align:center;">Verificar Código</h1>
      <p class="form-subtitle" style="text-align:center;">
        Introduz o código de 6 dígitos enviado para<br>
    ...
    <?php else: ?>
      ...
      <p class="form-subtitle" style="text-align:center;">Código verificado. Define a tua nova palavra-passe.</p>
      ...
      <form method="POST" novalidate>
        ...
        <div class="form-group">
          ...
          <input type="password" id="password" name="password" placeholder="Nova password segura" required autofocus>
